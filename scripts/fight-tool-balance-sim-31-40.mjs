// scripts/fight-tool-balance-sim-31-40.mjs
//
// Analog zu fight-tool-balance-sim-21-30.mjs, aber für Bracket 31-40: 1000
// zufällige Builds mit ECHTER Level-Progression (Level 31-40 uniform
// verteilt, Attributpunkte = (Level-1)*5, STAT_POINTS_PER_LEVEL=5) und
// gemischtem T2/T3/T4-Gear (TIER_DISTRIBUTION['31-40'] droppt gleichgewichtet
// T2/T3/T4, kein T1/T5 in diesem Bracket). Jeder Build durchläuft EIN
// Abenteuer mit fest 6 Monster-Kämpfen (HP/Mana/Schild nicht zurückgesetzt)
// gegen den VOLLEN Bracket-31-40-Pool (7 native + 7 hochskalierte
// "Veteran"-Monster aus 21-30), je RUNS_PER_BUILD mal wiederholt.
//
// Optional --calibrated: überschreibt hp/attack der 7 nativen Monster mit den
// Werten aus scripts/output/fight-tool-monster-curve-calibration-31-40.json
// (Trockenlauf-Ergebnis von fight-tool-monster-curve-calibration-31-40.mjs)
// für einen Vorher/Nachher-Vergleich ohne die echte JSON anzufassen.
//
// Nutzung: node scripts/fight-tool-balance-sim-31-40.mjs [buildCount] [runsPerBuild] [--calibrated]
...
const GEAR_TIERS = [2, 3, 4]; // TIER_DISTRIBUTION['31-40']: kein T1/T5
...
const ITEM_POOLS_BY_TIER = { 2: loadItemPoolsForTier(2), 3: loadItemPoolsForTier(3), 4: loadItemPoolsForTier(4) };
...
const SPELLS_2_4 = GEAR_TIERS.flatMap(loadSpellsForTier);
...
let MONSTERS_31_40_NATIVE = loadJson('public/mosters/dark-forest/dark-forest.31-40.json');
const MONSTERS_31_40_VETERAN = loadJson('public/mosters/dark-forest/dark-forest.31-40.veteran.json');

if (CALIBRATED) {
  const calibration = loadJson('scripts/output/fight-tool-monster-curve-calibration-31-40.json');
  MONSTERS_31_40_NATIVE = MONSTERS_31_40_NATIVE.map((m) => {
    const change = calibration.find((c) => c.name === m.name);
    return change ? { ...m, hp: change.newHp, attack: change.newAttack } : m;
  });
  console.log('⚙️  --calibrated aktiv: native Monster-hp/attack-Werte aus fight-tool-monster-curve-calibration-31-40.json übernommen.');
}

const MONSTER_POOL = [
  ...MONSTERS_31_40_NATIVE.map((m) => ({ ...m, origin: 'nativ' })),
  ...MONSTERS_31_40_VETERAN.map((m) => ({ ...m, origin: 'veteran' })),
];

function randInt(min, max) { return min + Math.floor(Math.random() * (max - min + 1)); }
function pick(arr) { return arr[randInt(0, arr.length - 1)]; }
function randomTier() { return GEAR_TIERS[randInt(0, GEAR_TIERS.length - 1)]; }
...
function randomBuild() {
  ...
  const shuffledSpells = [...SPELLS_2_4].sort(() => Math.random() - 0.5);
...
function summarizeBuild(build) {
  ...
  return {
    level: build.level, totalPoints: build.totalPoints, gear: gearNames, spells: spellNames,
    investedPoints: build.investedPoints, monsterSteps: build.monsterSteps.map((m) => m.name),
  };
}

console.log(`🎲 Generiere ${BUILD_COUNT} zufällige Bracket-31-40-Builds (Level 31-40, T2-4-Gear-Mix), je ${RUNS_PER_BUILD} Runs, ${FIGHTS_PER_ADVENTURE} Kämpfe/Abenteuer...`);
...
const levelBuckets = { '31-33': [], '34-37': [], '38-40': [] };
function levelBucketKey(level) { return level <= 33 ? '31-33' : level <= 37 ? '34-37' : '38-40'; }
...
for (let b = 0; b < BUILD_COUNT; b++) {
  ...
  buildResults.push({
    id: b, level: build.level, totalPoints: build.totalPoints, winRate,
    died: died / RUNS_PER_BUILD, timedOut: timedOut / RUNS_PER_BUILD, deathAtStep,
...
console.log('📈 WINRATE NACH LEVEL-BUCKET (innerhalb Bracket 31-40)');
...
const outPath = path.join(ROOT, 'scripts', 'output', `fight-tool-balance-report-31-40${suffix}.json`);
